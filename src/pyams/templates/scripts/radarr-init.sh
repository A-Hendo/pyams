...
echo "Starting Radarr initialization..."
...
(
  ...
  if grep -q "<ApiKey>.*</ApiKey>" "$CONFIG_FILE"; then
    ...
    # or it might have just the root tag. Radarr usually populates it.
    # But let's be safe and wait a bit more if it's missing.
    sleep 5
    sed -i "s|<ApiKey>.*</ApiKey>|<ApiKey>$API_KEY</ApiKey>|" "$CONFIG_FILE"
  fi

  echo "Waiting for Radarr API to become available..."

  # Wait for Radarr to be healthy (up to 2 minutes)
  MAX_RETRIES=24
  RETRY_COUNT=0
  ...
  if [ $RETRY_COUNT -eq $MAX_RETRIES ]; then
    echo "Error: Radarr API did not become available in time."
    exit 1
  fi

  echo "Radarr is up! Applying Zero-Config..."

  sleep 5

  # A. Setup Root Folder (/data/movies)
  ...
  echo "Radarr Zero-Config Complete!"
) &
